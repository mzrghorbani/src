...
namespace trackerTFP {
  ...
  private:
    ...
    EDGetTokenT<TTDTC::Streams> edGetTokenStubsDTC_;
    // ED input token of gp stubs
    EDGetTokenT<TTDTC::Streams> edGetTokenStubsGP_;
    EDGetTokenT<TTDTC::Streams> edGetTokenTracksGP_;
    // Setup token
  ...
  void Demonstrator::analyze(const Event& iEvent, const EventSetup& iSetup) {
    Handle<TTDTC::Streams> handleStubsDTC;
    iEvent.getByToken<TTDTC::Streams>(edGetTokenStubsDTC_, handleStubsDTC);
    stringstream ss;
    ss << header(dataFormats_->numChannel(Process::dtc));
    int nFrame(0);
    for (int region = 0; region < setup_->numRegions(); region++) {
      ss << infraGap(nFrame, dataFormats_->numChannel(Process::dtc));
      for (int frame = 0; frame < setup_->numFrames() + setup_->numFramesInfra(); frame++) {
        ss << this->frame(nFrame);
        for (int channel = 0; channel < dataFormats_->numChannel(Process::dtc); channel++) {
          const TTDTC::Stream& stream = handleStubsDTC->at(region * dataFormats_->numChannel(Process::dtc) + channel);
          TTDTC::BV bv;
    ...
    Handle<TTDTC::Streams> handleTracksGP;
    iEvent.getByToken<TTDTC::Streams>(edGetTokenTracksGP_, handleTracksGP);
    ss.str("");
    ss.clear();
    ...
    for (int region = 0; region < setup_->numRegions(); region++) {
      ...
      for (int frame = 0; frame < setup_->numFrames() + setup_->numFramesInfra(); frame++) {
        ...
        for (int channel = 0; channel < dataFormats_->numChannel(Process::gp); channel++) {
          const TTDTC::Stream& streamT = handleTracksGP->at(region * dataFormats_->numChannel(Process::gp) + channel);
          TTDTC::BV bv;
          if (frame < (int)streamT.size())
            bv = streamT[frame].second;
          ss << hex(bv);
          bv.reset();
          const TTDTC::Stream& streamS = handleStubsGP->at(region * dataFormats_->numChannel(Process::gp) + channel);
